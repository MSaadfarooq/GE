...
Heatmap(as.matrix(raw.dist), col = colz,
  ...
  bottom_annotation = columnAnnotation(Birthweight = vsd$bw, Sex=vsd$sex,
    col = list(Sex = c(Female = "pink2", Male = "steelblue"),
               Birthweight  = c(low = "brown", normal = "rosybrown")))
)
```
...
dds <- DESeqDataSet(gse, design = ~ sex + bw) # run with optimized design
...
sigDE <- DElist %>%   dplyr::filter(threshold=="Sig")
```
...
DElist %>%   
  ...
  geom_point(size = 0.5) + geom_hline(yintercept = -log10(0.05)) +
  geom_vline(xintercept = 1) + geom_vline(xintercept = -1) + my_theme
```
...
HM_genes <- res[order(res$padj), ] |>   head(100) |>   rownames() heatmapData <- assay(vsd)[HM_genes, ]  
# Scale counts for visualization 
heatmapData <- t(scale(t(heatmapData)))  
# Add annotation & colors 
heatmapColAnnot <- data.frame(colData(vsd)[, c("bw", "sex")]) heatmapColAnnot <- HeatmapAnnotation(df = heatmapColAnnot) 
colors <- colorRampPalette(rev(brewer.pal(9, "RdBu")))(255)  
# Plot as Heatmap 
Heatmap(heatmapData, col = colors, top_annotation = heatmapColAnnot,         cluster_rows = TRUE, show_row_names = FALSE, cluster_columns = T, column_dend_reorder = T)
```
...
top10_sigOE <- DElist %>%   arrange(padj) %>%   #pull(gene) %>% 		
  # Extract character vector of ordered genes   
  head(10)  
meta<-coldata %>% rename(sample=names)  
as_tibble(counts(dds[top10_sigOE$gene, ]), rownames = 'gene') %>%
