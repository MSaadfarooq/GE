...
```{r}
counts_SE |> 
  filter(.feature %in% c("CB0002","CB0017")) |> 
...
```{r}
se_norm_count <- gse |> identify_abundant(factor_of_interest = bw) |>  scale_abundance()
```
...
```{r}
SE.norm.Var <- counts_SE.norm |> keep_variable()
```

### PCA

```{r}
norm.PCA <- se_norm_count |> reduce_dimensions(method = "PCA", .dims = 3)
...
```{r}
gse |> test_differential_cellularity( ~ bw, .transcript = symbol)
```
